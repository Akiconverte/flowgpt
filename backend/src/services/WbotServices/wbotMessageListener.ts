...
const handleMessage = async (
  ...
  try {
    ...
    if (whatsapp.iaBot && !msg.key.fromMe && !ticket.isGroup) {
      const body = getBodyMessage(msg);
      if (body) {
        try {
          let response: string | null = null;
          if (whatsapp.geminiApiKey) {
            response = await handleGemini(body, whatsapp.geminiApiKey);
          } else if (whatsapp.ollamaUrl) {
          ...
          if (response) {
            ...
            await CreateLogTicketService({
              ...
              type: "iaBot",
              log: {
                message: `Mensagem respondida pela IA Direta. Fluxo interrompido.`
              }
            });
            return;
          }
          ...
          Sentry.captureException(e);
          logger.error(`Error in Direct AI Bot: ${e}`);
        }
      }
    }
            }
