...
import { handleOpenAi } from "./helpers/handleOpenAi";
import { handleGemini, handleOllama } from "../ExternalAIService";
  ...
  try {
    ...
    for (var i = 0; i < lengthLoop; i++) {
      ...
      if (nodeSelected.type === "gemini") {
        if (!whatsapp.geminiApiKey) {
          throw new AppError("Gemini API Key not configured for this connection.");
        }
        const prompt = nodeSelected.data.prompt || "";
        const response = await handleGemini(whatsapp.geminiApiKey, prompt);
        const bodyFila = formatBody(`${response}`, ticket.contact);
      ...
      if (nodeSelected.type === "ollama") {
        if (!whatsapp.ollamaUrl || !whatsapp.ollamaModel) {
          throw new AppError("Ollama settings not configured for this connection.");
        }
        const prompt = nodeSelected.data.prompt || "";
        const ollamaPrompt = whatsapp.ollamaPrompt
          ? `${whatsapp.ollamaPrompt}\n\n${prompt}`
          : prompt;
        const response = await handleOllama(
          whatsapp.ollamaUrl,
          whatsapp.ollamaModel,
          ollamaPrompt
        );
        const bodyFila = formatBody(`${response}`, ticket.contact);
